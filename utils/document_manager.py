...
class DocumentManager:
    ...
    def create_docx_files_dictionary(self, file_paths: List[str]) -> Dict[str, Document]:
        ...
        documents = {}
        logger.debug("Creating a dictionary of document parsers for the documents in the selected directory")
    ...
    def load_documents(self, file_paths: List[str]):
        logger.info("load_documents called.")
        ...
        logger.info(f"load_documents complete. Total document nodes: {self.total_document_nodes}, "
    ...
    def update_document_nodes(self, nodes: Dict[str, Dict[str, DocumentNode]]):
        ...
        logger.debug("Updating document nodes dictionary.")
    ...
    def generate_embeddings(self, nodes: Dict[str, Dict[str, DocumentNode]]):
        ...
        logger.debug("Generating embeddings for nodes.")
    ...
    def generate_node_embeddings(self, nodes: Dict[str, Dict[str, DocumentNode]]):
        logger.debug("Generating node embeddings.")
        for file_path, node_dict in nodes.items():
            for node_id, node in node_dict.items():
                logger.info("Generating node embedding for: " + node.body_text)
                generate_embedding(node)
                logger.info("Node embedding generated.")
        ...
        for file_path, node_dict in nodes.items():
            for node_id, node in node_dict.items():
                for sentence in node.sentence_list:  # Access the sentence_list directly
                    logger.info("Generating sentence embedding for: " + sentence.text)
    ...
    def save_nodes_body_text(self, file_path):
        logger.info(f"Saving nodes body text to {file_path}.")
    ...
    def save_sentence_text(self, file_path):
        logger.info(f"Saving sentence text to {file_path}.")
        with open(file_path, 'w') as f:
            for node in self.document_nodes.values():
    ...
    def print_first_five_words_of_nodes(self):
        logger.debug("Printing first five words of each node.")
        for node_id, node in self.document_nodes.items():
            ...
            logger.info(f'{node_id} : {first_five_words_str}')

    def print_embedding_order(self):
        logger.debug("Printing embedding order.")
        for data_type in ['node', 'sentence']:
            logger.info(f"Embedding order for {data_type}s:")
            for embedding_type, id in self.embedding_order[data_type]:
                logger.info(f"{embedding_type} ID: {id}\n")

    def build_faiss_index(self):
        logger.debug("Building FAISS index.")
        for data_type in ['node', 'sentence']:
            embeddings = []
            logger.info(f"Length of {data_type} embedding_order before building the index:",
    ...
    def search_similar_nodes(self, query_embedding: List[float], k: int, data_type='node') -> \
        ...
        logger.debug(f"Searching for similar nodes with data type: {data_type}.")
    ...
    def save_manager_state(self, output_directory: str):
        logger.info(f"Saving manager state to {output_directory}.")
    ...
    def load_manager_state(self, input_directory: str):
        logger.info(f"Loading manager state from {input_directory}.")
